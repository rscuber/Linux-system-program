...
void MyTask::finish_the_work(void)
{
   ...
   */
    //open file with O_RDWR, O_CREAT, O_EXCL
    ...
    //write to a new file named "letter".
    if (write(fd, buf, data_.size()) < 0) {
        handle_error("write1");
    }

    //Write pk_ to a file named "key.pub" in string
    int fd1 = open("key.pub", O_RDWR | O_CREAT | O_EXCL, 0644);
    if (fd1 < 0) handle_error("open1");
    ...
    //Write sk_ to a file named "key.prv" in string
    int fd2 = open("key.prv", O_RDWR | O_CREAT | O_EXCL, 0644);
    if (fd2 < 0) handle_error("open2");
    memset(buf, 0, sizeof(buf));
    sprintf(buf, "%" PRId64 "\n%" PRId64, sk_, sn_);
